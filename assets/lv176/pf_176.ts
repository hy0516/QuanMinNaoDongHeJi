...
@ccclass
export default class pf_176 extends cc.Component {
    ying = true;
    pfk: cc.Node = null;
    ...
    refreshUI() {
        for (let i = 0; i < this.pfk.childrenCount; i++) {
            ...
            if (i != 0) btn.getChildByName("luxiang").active = data_176.unlocklist.indexOf(btn.name) == -1
            btn.getChildByName("syz").active = data_176.usetou == btn.name;
        }
    }
    oninit(ying: boolean) {
    ...
    unlockLv(even: cc.Event.EventTouch) {
        ...
        if (even.currentTarget.getChildByName("luxiang").active) {
            VideoManager.getInstance().showVideo(() => {
                ...
                data_176.unlocklist.push(even.currentTarget.name);
            })
        } else {
            data_176.usetou = even.currentTarget.name;
        }
