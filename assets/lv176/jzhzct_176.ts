...
import data_176 from "./data_176";
import end_176 from "./end_176";
import mapitem_176 from "./mapitem_176";
...
@ccclass
export default class jzhzct_176 extends BaseGame {
    @property(cc.Label)
    time: cc.Label = null;
    @property(cc.String)
    nodename: string = "";
    ...
    tiezhinum = 0;
    // step = 1;
    gracolor: string[] = ["#a5ff00", "#ffe900", "#ffc633", "#808080", "#b100ff"];
    ...
    onLoad() {
        ...
        this.scheduleOnce(() => {
            AudioManager.playMusic("关卡背景_lv176");
        }, 0.5)
        ...
        this.node.on("gamewin_176", () => {
            this.checknextlv();
            // }, 1.5);

        }, this)
        for (let i = 0; i < this.map.childrenCount; i++) {
            var script = this.map.children[i].getComponent(mapitem_176);
            if (script.startnode) {
        ...
        // var index = ""
        // if(this)
        // console.log("1" + this.curlvname)
        // console.log("2" + this.nodename)
        var list = this.nodename.split("_");
        var lvnum = "1";
        if (list.length == 2) lvnum = "1";
        else lvnum = list[2];
        if(GameData.curFenLei!=""){
             common.openLuJing("主页-" + "第" + GameData.curFenLei + "页" + "-" + "节奏盒子长条" + "-第" + lvnum + "关");
        }else{
             common.openLuJing("主页-" + "节奏盒子长条" + "-第" + lvnum + "关");
        }
       
    }
    loadTou() {
        this.tou.children[0].destroy();
        var namelist = data_176.usetou.split("xk");
        AssetManager.load(GameData.curGameStyle, "tou_sk", cc.Prefab, null, (icon: cc.Prefab) => {
    ...
    checkWin() {
        ...
        for (let i = 0; i < this.map.childrenCount; i++) {
            ...
            if (!script.isuse && !script.qiang && cc.Intersection.lineRect(new cc.Vec2(toumappoi.x, toumappoi.y), poi, boxrect)) {
                AudioManager.playEffect("移动_lv176");
                this.isgo = false;
                return;
            }
            poi = new cc.Vec2(toumappoi.x + this.tou.width, toumappoi.y);
            ...
            if (i == this.map.childrenCount - 1) {
                if (!isall) {
                    AssetManager.load(GameData.curGameStyle, "xy_ske", cc.Prefab, null, (UI: cc.Prefab) => {
                        AudioManager.playEffect("撞晕_lv176");
    ...
    checkBox() {
        ...
        for (let i = 0; i < this.map.childrenCount; i++) {
            ...
            if (!script.isuse && !script.qiang && cc.Intersection.lineRect(new cc.Vec2(toumappoi.x, toumappoi.y), poi, boxrect) && this.pathList.indexOf(this.map.children[i]) == -1) {
                ...
                this.map.children[i].getComponent(mapitem_176).isuse = true;
    ...
    onwin() {
        var handler = () => {
            ...
            this.scheduleOnce(() => {
                if (this.nextLv) {
                    AssetManager.load(GameData.curGameStyle, "end_176", cc.Prefab, null, (UI: cc.Prefab) => {
                        var UINode = cc.instantiate(UI);
                        UINode.parent = this.node;
                        UINode.getComponent(end_176).oninit(true);
                    })
                } else {
                    this.loadend();
                }
    ...
    onlost() {
        ...
        this.scheduleOnce(() => {
            AssetManager.load(GameData.curGameStyle, "end_176", cc.Prefab, null, (UI: cc.Prefab) => {
                var UINode = cc.instantiate(UI);
                UINode.parent = this.node;
                UINode.getComponent(end_176).oninit(false);
            })
        }, 0.7);
    }
    ...
    BtnHandler(event: cc.Event.EventTouch) {
        ...
        switch (event.currentTarget.name) {
            ...
            case "btn_pf":
                if (GameData.PauseGame == true) return;
                AssetManager.load(GameData.curGameStyle, "pfpanel", cc.Prefab, null, (UI: cc.Prefab) => {
                    var UINode = cc.instantiate(UI);
                    UINode.parent = this.node;
                })
                break;
