import Sheep_CorrallingFlock309 from "./Sheep_CorrallingFlock309";
import Wolf_CorrallingFlock309 from "./Wolf_CorrallingFlock309";

import Shepherd_CorrallingFlock309 from "./Shepherd_CorrallingFlock309";
import GameViewLogic_CorallingFlock309 from "./GameViewLogic_CorrallingFlock309";
import AnswerLogic_CorrallingFlock309 from "./AnswerLogic_CorrallingFlock309";
import CircleNode_CorrallingFlock309 from "./CircleNode_CorrallingFlock309";
import DX_Config309 from "./DX_Config309";
import DX_EventType309 from "./DX_EventType309";
import GameData from "../script/common/GameData";
import AudioManager from "../script/common/AudioManager";
import BaseGame from "../script/common/BaseGame";
import VideoManager from "../script/common/VideoManager";
import RouteLogic_CorrallingFlock309 from "./RouteLogic_CorrallingFlock309";
...
@ccclass
export default class LevelCtrl_CorrallingFlock309  extends BaseGame {
    ...
    private gameViewLogic: GameViewLogic_CorallingFlock309 = null;
    private sheepArr: Sheep_CorrallingFlock309[] = null;
    private wolfArr: Wolf_CorrallingFlock309[] = null;
    ...
    onLoad() {
        ...
        this.shepherd = this.map.getComponentInChildren(Shepherd_CorrallingFlock309).node;
        ...
        this.gameViewLogic = this.map.getChildByName("GameViewLogic").getComponent(GameViewLogic_CorallingFlock309);
        this.sheepArr = this.map.getComponentsInChildren(Sheep_CorrallingFlock309);
        this.wolfArr = this.map.getComponentsInChildren(Wolf_CorrallingFlock309);
    ...
    start() {
        ...
        this.node.on(DX_EventType309.CorrallingFlock_OnDrawFinish, this.onDrawFinish, this);


        // 
        this.node.on(DX_EventType309.CrossTheRiver_OnGameFail, this.onGameFailEvent, this);

        if(this.gameViewLogic.getComponentInChildren(AnswerLogic_CorrallingFlock309)){
            this.gameViewLogic.getComponentInChildren(AnswerLogic_CorrallingFlock309).onDrawAnswer(DX_Config309.CorrallingFlock_Guide);
        }
    }
    onDestroy() {
        this.node.off(DX_EventType309.CorrallingFlock_OnDrawFinish, this.onDrawFinish, this);
        this.node.off(DX_EventType309.CrossTheRiver_OnGameFail, this.onGameFailEvent, this);
    ...
    onDrawFinish(event: cc.Event.EventCustom) {
        ...
        if(this.gameViewLogic.getComponentInChildren(AnswerLogic_CorrallingFlock309)){
            this.gameViewLogic.getComponentInChildren(AnswerLogic_CorrallingFlock309).node.destroy();
        }
    }
    ...
    onwin() {
        this.iswin = true;
        this.gou.cleanup();
        this.pig_ske.playAnimation("tw", 0);
        cc.tween(this.gou)
    ...
    update() {
        if (this.m_isDrawFinish) {
            ...
            for (let i = 0; i < this.circleArr.length; i++) {
                ...
                this.circleArr[i].getComponent(CircleNode_CorrallingFlock309).onShorted(startPos, endPos);
            }
        ...
        while (worldPoints.length > 0) {
            ...
            while (nextPos != currPos) {
                ...
                if (sub.mag() >= this.m_circleDiameter) {
                    ...
                    circle.getComponent(CircleNode_CorrallingFlock309).setShepherd(this.shepherd);
                    // 设置距离关节
                    if (lastCircle) {
