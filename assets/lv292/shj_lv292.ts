...
import item_move_lv292 from "./item_move_lv292";
const { ccclass, property } = cc._decorator;

@ccclass
export default class shj_lv292 extends BaseGame {
    @property(cc.Node)
    ...
    tipsGroupFilter(puzzNode:cc.Node){
        ...
        // 停止目标节点的闪烁动画
        let targetNode = puzzNode.getComponent(item_move_lv292).targetNode;
        if (targetNode) {
    ...
    tipEffect(){
        ...
        let target = puzz.getComponent(item_move_lv292).targetNode;

        //若状态不正确,反转
        if(puzz.getComponent(item_move_lv292).stateCreccot == false){
            puzz.getComponent(item_move_lv292).puzzTurn();
        }
        target.active = true;
        target.opacity = 255;
        puzz.opacity = 255;
    ...
    puzzInof(){
        ...
        for(let i = 0; i < puzzs.length; i++ ){
            let puzz = puzzs[i];
            puzz.addComponent(item_move_lv292);
            puzz.getComponent(item_move_lv292).puzzInof(this.node);
            ...
            if(name.slice(-2) == "jx"){
                puzz.scaleX = -1;
                //默认翻转状态为 false
                puzz.getComponent(item_move_lv292).stateCreccot = false;
            }
    ...
    getPuzzTargetNode(puzzNode:cc.Node){
        ...
        for(let i = 0 ; i < skins.length ; i++ ){
            ...
            if(pointNode.name == puzzNode.name){
                ...
                puzzNode.getComponent(item_move_lv292).targetNode = pointNode;
               // console.log(puzzNode.name,puzzNode.getComponent(item_move_lv292).targetNode);
            }
        }
    }
