...
import shj_lv292 from "./shj_lv292";


const { ccclass, property } = cc._decorator;

@ccclass
export default class itme_move_lv292 extends cc.Component {
 ...
 // @property(cc.Integer)
   ...
   startIndex = 0;
    ...
    puzzInof(main:cc.Node){
        ...
        //获取目标节点
        this.mainNode.getComponent(shj_lv292).getPuzzTargetNode(this.node);
    ...
    onTouchEnd(event: cc.Event.EventTouch) {
        ...
        if(this.posInof(event)){
            ...
            //拼图完成+1
            this.mainNode.getComponent(shj_lv292).curPuzz_num += 1;

            this.mainNode.getComponent(shj_lv292).updatelv();


            //播放音效
            AudioManager.playEffect("right292");
           
            //关闭闪烁
            this.mainNode.getComponent(shj_lv292).tipLock = false;
            //剔除当前节点
            this.mainNode.getComponent(shj_lv292).tipsGroupFilter(this.node);

            //提示开锁
            this.mainNode.getComponent(shj_lv292).lock_clickDouble = true;
