    ...
    onBeginContact(contact,selfCollider, otherCollider) {
      ...
      //  const Manifold = contact.getManifold();
        ...
        if (isContactYOver&&!this.islost) {
            this.islost=true;
            this.node.dispatchEvent(new cc.Event.EventCustom(HCSHJ_EventType_251.GAMELOST_EVENT, true));
        }
        ...
        if(selfCollider.node.name!=otherCollider.node.name)return;
         ...
         let cusEvent = new cc.Event.EventCustom(HCSHJ_EventType_251.CONTACTCOLLIDER_EVENT, true);
