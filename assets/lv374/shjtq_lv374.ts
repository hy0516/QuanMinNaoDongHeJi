...
import shjtq_lv374_move from "./shjtq_lv374_move";
...
@ccclass
export default class shjtq_lv374 extends BaseGame {
    ...
    /** 7 号彩蛋：先与普通角色同套圈拉绳，再挂到 ch 演出；结算前挂回并恢复 UI / BGM */
    private sevenEggParent: cc.Node = null;
    private sevenEggSiblingIndex = 0;
    private sevenSavedLocalPos: cc.Vec2 = null;
    private eggUiBackup: { node: cc.Node; wasActive: boolean }[] = [];
    /** 7 号彩蛋层正在展示，避免误点把 ch 关掉 */
    private is7EggSequence = false;
    ...
    onLoad() {
        ...
        this.scheduleOnce(() => {
            AudioManager.playMusic("bgmlv374", true, 0.7);
        }, 0.5);
    ...
    /** 7：挂到 ch、藏界面与敌人列、切彩蛋 BGM；失败返回 false（需由调用方恢复抓捕） */
    private begin7EggSequence(element: cc.Node): boolean {
        const chNodeEarly = this.bgNode.getChildByName(`ch`);
        if (!chNodeEarly) {
            cc.error(`[shjtq_lv374] ch 节点缺失`);
            return false;
        }

        this.sevenEggParent = element.parent;
        this.sevenEggSiblingIndex = element.getSiblingIndex();
        this.sevenSavedLocalPos = cc.v2(element.x, element.y);

        this.eggUiBackup.length = 0;
        const pushUi = (n: cc.Node) => {
            if (!n) return;
            this.eggUiBackup.push({ node: n, wasActive: n.active });
            n.active = false;
        };
        pushUi(this.tittle);
        pushUi(this.addtimetips);
        const timeNd = this.bgNode.getChildByName(`timeNode`);
        if (timeNd) pushUi(timeNd);
        pushUi(this.bgNode.getChildByName(`fanhui`));
        pushUi(this.bgNode.getChildByName(`jiashi`));
        pushUi(this.bgNode.getChildByName(`jiansu`));
        const tishiNd = this.bgNode.getChildByName(`tishi`);
        if (tishiNd) pushUi(tishiNd);
        pushUi(this.bgNode.getChildByName(`nodeCount`));

        this.bgNode.getChildByName(`enemNode`).active = false;
        const wpos = this.sevenEggParent.convertToWorldSpaceAR(cc.v2(element.x, element.y));
        element.removeFromParent(false);
        chNodeEarly.addChild(element);
        element.setPosition(chNodeEarly.convertToNodeSpaceAR(wpos));
        element.setSiblingIndex(chNodeEarly.childrenCount - 1);
        chNodeEarly.active = true;
        this.is7EggSequence = true;

        AudioManager.playMusic(`caidan_bgm`, true, 0.5);
        return true;
    }

    /** 7：从 ch 挂回 enemNode（先隐藏角色）、恢复敌人列与顶部 UI、切回关卡 BGM */
    private end7EggBeforeJieSuan(element: cc.Node): void {
        if (!this.sevenEggParent || !this.sevenSavedLocalPos) return;

        const chNode = this.bgNode.getChildByName(`ch`);
        if (chNode) chNode.active = false;

        element.removeFromParent(false);
        this.sevenEggParent.addChild(element);
        element.setPosition(this.sevenSavedLocalPos);
        element.setSiblingIndex(this.sevenEggSiblingIndex);
        element.active = false;

        this.bgNode.getChildByName(`enemNode`).active = true;

        for (const rec of this.eggUiBackup) {
            rec.node.active = rec.wasActive;
        }
        this.eggUiBackup.length = 0;
        this.sevenEggParent = null;
        this.sevenSavedLocalPos = null;
        this.is7EggSequence = false;

        AudioManager.playMusic(`bgmlv374`, true, 0.5);
    }

    /** 套中后的展示/结算（主界面绳子关掉，showSk 里 djwtq 播对应 zl） */
    private openCatchJieSuan(element: cc.Node): void {
        if (element.name === `7_tq`) {
            this.end7EggBeforeJieSuan(element);
        }
    ...
    catchBtn(event: cc.Event.EventTouch) {
        ...
        this.addOneTimeListener(tqDisplay, () => {
            ...
            element.getComponent(shjtq_lv374_move).canwalk = false;
            ...
            this.addOneTimeListener(enemArm, () => {
                if (element.name === `7_tq`) {
                    if (!this.begin7EggSequence(element)) {
