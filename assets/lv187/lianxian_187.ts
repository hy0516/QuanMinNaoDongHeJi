...
import item_lianxian_187 from "./item_lianxian_187";
import lianxian_config_187 from "./lianxian_config_187";
import lianxian_config187 from "./lianxian_config_187";
...
@ccclass
export default class game_wenzi_187 extends BaseGame {
    ...
    /**已连线节点数组 (用于线条重绘)*/
    public okList: item_lianxian_187[] = [];
    public maxlv = 3;
    public curlv = 1;
    ...
    onLoad() {
        ...
        this.scheduleOnce(()=>{
            AudioManager.playMusic("bgm_187");
        },0.5);
        ...
        if (!this.curStartItem || this.okList.length == 1) {
            ...
            for (let i = 0; i < childlist.length; i++) {
                ...
                if (childitem.getBoundingBox().contains(this.content.convertToNodeSpaceAR(event.getLocation()))) {
                    var thing = childitem.getComponent(item_lianxian_187);
                    //点击过的节点，跳过
    ...
    onTouchEnd() {
        ...
        if (this.okList.length == 1) {
            var thing = this.okList[0].getComponent(item_lianxian_187);
    ...
    labelUpdate(){
        this.Lable.string = " " + this.curlv + " " + "/" + " 3" ;
    }
    loadendlost() {
