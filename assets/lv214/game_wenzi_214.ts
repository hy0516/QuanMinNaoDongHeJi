...
import item_wz_214 from "./item_wz_214";
import wenzi_config_214 from "./wenzi_config_214";
...
export default class game_wenzi extends BaseGame {
    ...
    onLoad() {
        ...
        for (let i = 0; i < wenzi_config_214.level1.mapdata.length; i++) {
            for (let j = 0; j < wenzi_config_214.level1.mapdata[i].length; j++) {
                var data = wenzi_config_214.level1.mapdata[i][j];
                var item = cc.instantiate(this.item);
                item.getComponent(item_wz_214).oninit(data);
    ...
    onTouchMove(event: cc.Event.EventTouch) {
        ...
        for (let i = 0; i < childlist.length; i++) {
            ...
            if (childitem.getBoundingBox().contains(this.content.convertToNodeSpaceAR(event.getLocation()))) {
                var thing = childitem.getComponent(item_wz_214);
    ...
    checkAnswerd(list: string) {
        if (wenzi_config_214.levelanswerd.indexOf(list) != -1) {
            GameData.wenziWinNum++;
            AudioManager.playEffect(wenzi_config_214.levelSound[this.curId - 1]);
            // console.log(GameData.wenziWinNum);
            if (GameData.wenziWinNum == wenzi_config_214.levelanswerd.length) {
                // console.log("赢了");
    ...
    cuowu() {
        ...
        if (this.iscuowu) return
        this.iscuowu = true;
        AudioManager.playEffect(wenzi_config_214.lineLost);
        this.dacha();
        setTimeout(() => {
    ...
    BtnHandler(even: cc.Event.EventTouch) {
        ...
        switch (even.currentTarget.name) {
            case "btn_close":
                this.node.cleanup();
                AudioManager.playEffect(wenzi_config_214.button_wz);
            ...
            case "btn_tips":
                ...
                var handlers = () => {
                    AssetManager.load(GameData.curGameStyle, "TipPanel_wz_214", cc.Prefab, null, (name: cc.Prefab) => {
    ...
    loadend() {
        ...
        AssetManager.load(GameData.curGameStyle, "endwin_wz_214", cc.Prefab, null, (name: cc.Prefab) => {
    ...
    Timeing() {
        ...
        if (GameData.startTime == 0) {
            ...
            this.scheduleOnce(() => {
                // this.loadend();
                AssetManager.load(GameData.curGameStyle, "endlost_wz_214", cc.Prefab, null, (name: cc.Prefab) => {
                    var endNode = cc.instantiate(name);
                    endNode.parent = cc.find("Canvas");
                    endNode.opacity = 0;
                    cc.tween(endNode)
