...
export interface ILv449MainForLostPanel {
...
@ccclass
export default class lostPanel_lv449 extends cc.Component {
    public mainGame: ILv449MainForLostPanel = null;
    ...
    onClose() {
        ...
        cc.resources.load("prefabs/common/Hall", cc.Prefab, (err, hall: cc.Prefab) => {
            if (err || !hall) {
                cc.error("[lostPanel_lv449] 加载 Hall 失败", err?.message);
                return;
            }
    ...
    onFuhuo() {
        ...
        VideoManager.getInstance().showVideo(
            ...
            () => {
                AudioManager.resumeMusic();
                cc.warn("[lostPanel_lv449] 广告调用失败");
            }
        );
    }
    ...
    onRestart() {
        ...
        AssetManager.load(GameData.curGameStyle, GameData.curGameName, cc.Prefab, null, (prefab: cc.Prefab) => {
            if (!prefab) {
                cc.error("[lostPanel_lv449] 加载关卡失败");
                return;
            }
