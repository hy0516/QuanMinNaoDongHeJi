...
import lostPanel_lv449, { ILv449MainForLostPanel } from "./lostPanel_lv449";
import PuzzlePiece449 from "./puzzle_piece_lv449";
...
@ccclass
export default class pt_lv449 extends BaseGame implements ILv449MainForLostPanel {
    canjiaohu = false;
     ...
     * 留空则沿用分包资源 `picture_lv449/level/n/nmovie`。
    ...
    /** 小关过渡黑幕（通关视频结束后）：淡入→停顿→淡出后关闭；不拖则按名 heiNode 查找 */
    @property(cc.Node)
    ...
    private _cellNodes: PuzzlePiece449[][] = [];
    private _uf: UnionFind449 = null;
    private _sourceTex: cc.Texture2D = null;
    /** 开局翻牌：牌背 `picture_lv449/kp` */
    ...
    /** 抖音离屏 / OPPO·Vivo 等快游戏 qg.createVideo（逻辑见 TtOffscreenVideoHelper） */
    ...
    /** 图块距离 playArea 底图边框的留白 */
    private readonly _playAreaPadding = 8;
    ...
    /** >0 表示本局 buildGrid 已跳过开局翻牌，待黑幕结束后再翻 */
    private _deferredOpeningFlipSeq = 0;
    /** 黑幕淡出与 grid 就绪都满足后再翻牌 */
    ...
    onLoad() {
        ...
        this.canjiaohu = true;
        this.scheduleOnce(() => {
            AudioManager.playMusic(`bgmlv449`, true, 0.5);
        }, 0.5);

        this.bindHelpButtons();
        this.schedule(this.tickRoundTimer, 1);
        this.resetHeiNodeVisual();
        this.startStage(1);
    }
    ...
    onDestroy() {
        ...
        const hei = this.resolveHeiNode();
        if (hei) {
            cc.Tween.stopAllByTarget(hei);
        }
    ...
    /** @param deferOpeningFlip 为 true 时仅铺牌背，黑幕淡出后再翻牌 */
    private startStage(stage: 1 | 2 | 3, deferOpeningFlip = false) {
        ...
        this.clearPlayArea();
        const imgPath = `picture_lv449/level/${stage}/${stage}`;
        AssetManager.load(GameData.curGameStyle, imgPath, cc.Texture2D, null, (tex: cc.Texture2D) => {
            ...
            if (bundle) {
                bundle.load("picture_lv449/kp", cc.Texture2D, (err: Error, kpTex: cc.Texture2D) => {
    ...
    private buildStencilFrameFromCornerConvex(
        ...
        if (!tex.initWithData(data, cc.Texture2D.PixelFormat.RGBA8888, iw, ih)) {
            cc.warn("[pt_lv449] 圆角遮罩纹理 initWithData 失败");
            tex.destroy();
            return null;
        }
    ...
    private buildGrid(deferOpeningFlip = false) {
        const gs = this._gridSize;
        const n = gs * gs;
        const w = this.playArea.width - this._playAreaPadding * 2;
        const h = this.playArea.height - this._playAreaPadding * 2;
        if (w <= 0 || h <= 0) {
            cc.warn("[pt_lv449] playArea 宽高需大于留白");
            return;
        }
        this._cellW = w / gs;
        this._cellH = h / gs;
        ...
        for (let r = 0; r < gs; r++) {
            for (let c = 0; c < gs; c++) {
                ...
                const comp = node.getComponent(PuzzlePiece449);
    ...
    private resolveHeiNode(): cc.Node {
        if (this.heiNode && this.heiNode.isValid) return this.heiNode;
        return this.node.getChildByName("heiNode");
    }

    private resetHeiNodeVisual() {
        const n = this.resolveHeiNode();
        if (!n) return;
    ...
    private showLostPanel() {
        AssetManager.load(GameData.curGameStyle, "lostPanel_lv449", cc.Prefab, null, (prefab: cc.Prefab) => {
            if (!prefab) {
                cc.error("[pt_lv449] 加载 lostPanel_lv449 失败");
            ...
            lostNode.opacity = 0;
            const lostScript = lostNode.getComponent(lostPanel_lv449) || lostNode.addComponent(lostPanel_lv449);
    ...
    private openYtPreview() {
        ...
        if (!root || !spNode) {
            cc.warn("[pt_lv449] ytNode/showpic 未找到");
            return;
        }
        const sp = spNode.getComponent(cc.Sprite);
        if (!sp) {
            cc.warn("[pt_lv449] showpic 需带 Sprite");
            return;
        }
    ...
    private createPieceNode(correctId: number): cc.Node {
        ...
        const comp = root.addComponent(PuzzlePiece449);
    ...
    private tryRigidGroupTranslation(
        ...
        const oldN: PuzzlePiece449[][] = [];
        for (let r = 0; r < gs; r++) {
            oldN[r] = this._cellNodes[r].slice();
        }
        const newCompAt: PuzzlePiece449[][] = [];
        for (let r = 0; r < gs; r++) {
    ...
    /**
     * 通关视频结束后：0.2s 淡入全黑时切下一关 → 停 1s → 0.2s 淡出。
     * 无 heiNode 时直接切关。
     */
    private runStageHeiTransition(onFullBlack: () => void, onFadeOutComplete: () => void) {
        const n = this.resolveHeiNode();
        if (!n || !n.isValid) {
            onFullBlack();
            return;
    ...
    private onStageClear() {
        ...
        const onVideoFlowDone = () => {
            ...
            if (st < pt_lv449.stageTotal) {
                const next = (st + 1) as 1 | 2 | 3;
                const hei = this.resolveHeiNode();
                if (hei && hei.isValid) {
                    this.runStageHeiTransition(
                        () => {
                            GameData.PauseGame = false;
    ...
    /**
     * 抖音离屏 / OPPO·Vivo 等 qg.createVideo：见 TtOffscreenVideoHelper（TT 含 scaleY 修正倒置）
     */
    private runTtOffscreenStageVideo(
        ...
        if (!host.activeInHierarchy) {
            cc.warn("[pt_lv449] videoHost 仍不在激活链上，请检查场景根或父节点");
        }
    ...
    private resolveVideoHost(): { host: cc.Node; vp: cc.VideoPlayer } | null {
        ...
        if (!vp) {
            cc.warn("[pt_lv449] videoHost 上无 VideoPlayer");
            return null;
        }
        return { host, vp };
    }
    ...
    private runStageVideoPlayer(done: () => void, opts: { localAsset?: cc.Asset; remoteUrl?: string }) {
        ...
        if (this.isTtRuntime()) {
            ...
            cc.warn(
                "[pt_lv449] 未解析到可播放的视频 URL（抖音/快游戏请配置「阶段视频远程目录」stageVideoRemoteBase 或 https）；将回退 cc.VideoPlayer"
            );
        }
    ...
    private playStageVideo(stage: number, done: () => void) {
        ...
        const path = `picture_lv449/level/${stage}/${stage}movie`;
        AssetManager.load(GameData.curGameStyle, path, cc.Asset, null, (asset: cc.Asset) => {
            ...
            if (!asset) {
                cc.warn("[pt_lv449] 视频加载失败，跳过:", path);
                this.scheduleOnce(done, 0.3);
                return;
            }
    ...
    fanhui() {
        ...
        cc.resources.load("prefabs/common/Hall", cc.Prefab, (err, UI: cc.Prefab) => {
            if (err || !UI) {
                cc.error("[pt_lv449] 加载 Hall 失败", err?.message);
                return;
            }
