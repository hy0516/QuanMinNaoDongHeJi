...
import qsh_lv248 from "./qsh_lv248";
const { ccclass, property } = cc._decorator;

@ccclass
export default class ddl_lv248 extends BaseGame {
    @property(cc.Node)
    ...
    onLoad() {
        ...
        if(cc.sys.localStorage.getItem("lv248totalRank")&&cc.sys.localStorage.getItem("lv248totalRank")>0)
        {
            this.totalRank=cc.sys.localStorage.getItem("lv248totalRank");
    ...
    gamestart()
    {
        AudioManager.playMusic(`bgmlv248`);
    ...
    UpdateNewNode()
    {
        ...
        if(this.totalNodeNum>0&&this.totalNodeNum%5==0)
        {
          this.curNewNode.getComponent(qsh_lv248).movespeed=8+randomSpeed*4;
        }
        this.bgNode.addChild(this.curNewNode);
        this.curNewNode.x=Math.random()>0.5?-600:600;
        if (this.curNewNode.x === 600) { 
            this.curNewNode.scaleX = -0.8;
        } else {
            this.curNewNode.scaleX = 0.8;
        }
        this.curNewNode.y=-520+(this.totalNodeNum>3?3:this.totalNodeNum)*60;
    ...
    clickPlayerJump(even: cc.Event.EventTouch)
    {
        if(this.canjiaohu)
        {
            ...
            this.player.getComponent(player_lv248).triggerJump();
        ...
        switch (even.currentTarget.name) {
            case "blxx":
              this.lostpanel.active=false;
                ...
                if(!cc.sys.localStorage.getItem("lv248totalNum"))
                {
                    cc.sys.localStorage.setItem("lv248totalNum",this.totalNodeNum-1);
                }else if(cc.sys.localStorage.getItem("lv248totalNum")<this.totalNodeNum-1)
                {
                    cc.sys.localStorage.setItem("lv248totalNum",this.totalNodeNum-1);
                }
                this.node.getChildByName(`jsPanel`).getChildByName(`overnode`).getChildByName(`topheight`).getComponent(cc.Label).string=`${cc.sys.localStorage.getItem("lv248totalNum")}`;
                this.node.getChildByName(`jsPanel`).getChildByName(`overnode`).getChildByName(`toprank`).getComponent(cc.Label).string=`${cc.sys.localStorage.getItem("lv248totalRank")}`;
            ...
            case "ljfh":
                VideoManager.getInstance().showVideo(()=>{
                   this.curNewNode.x=0;
                   this.player.x=0;
                   this.player.y=this.curNewNode.y+60;
                   ...
                   this.player.getComponent(player_lv248).curY=this.player.y;
                   this.successEvent(true);
                   this.lostpanel.active=false;
                });
            break;
    ...
    successEvent(isperfect=false)
    {
        ...
        if(this.curRank>this.totalRank)
        {
            ...
            this.bgNode.getChildByName(`fsk`).getChildByName(`toprank`).getComponent(cc.Label).string=this.totalRank;
            cc.sys.localStorage.setItem("lv248totalRank",this.totalRank);
        }
         if(this.bgNode.getChildByName(`fd`).active)
        {
            if(this.bgNode.getChildByName(`fd`).y<=330)
                ...
                {
                    cc.tween(this.bgNode.getChildByName(`fd`)).delay(0.6).to(0.3,{y:this.bgNode.getChildByName(`fd`).y-60}).call(()=>{}).start();
                }
            
        }
        else if((this.totalNodeNum%50)-40==0&&(!this.player.children[1].active||!this.player.children[2].active))//
        {
            this.bgNode.getChildByName(`fd`).active=true;
            this.bgNode.getChildByName(`fd`).y=600+60;
    ...
    showAndDestoryPre(state:number)
    {
        ...
        tempPre.x=this.player.x+Math.random()>0.5?(50+Math.random()*60):-(50+Math.random()*60);
        tempPre.y=this.player.y+50+Math.random()*60;
    ...
    updateCerate()
    {
        ...
        {
            if(this.bgNode.getChildByName(`bj`).y>-1500)this.tweenNode151(this.bgNode.getChildByName(`bj`));
            this.tweenNode151(this.player);
            this.player.getComponent(player_lv248).curY-=60;
    ...
    {
        cc.tween(aNode).to(0.3,{y:aNode.y-60}).start();
    }
