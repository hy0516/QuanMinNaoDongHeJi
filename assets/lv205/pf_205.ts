...
@ccclass
export default class pf_205 extends cc.Component {
    ying = true;
    pfk: cc.Node = null;
    ...
    refreshUI() {
        for (let i = 0; i < this.pfk.childrenCount; i++) {
            ...
            //该按钮不存在会返回true，开启luxiang图标
            if (i != 0) btn.getChildByName("luxiang").active = data_205.unlocklist.indexOf(btn.name) == -1;
            btn.getChildByName("syz").active = data_205.usetou == btn.name;
        }
    }
    oninit(ying: boolean) {
    ...
    unlockLv(even: cc.Event.EventTouch) {
        ...
        if (even.currentTarget.getChildByName("luxiang").active) {
            VideoManager.getInstance().showVideo(() => {
                ...
                data_205.unlocklist.push(even.currentTarget.name);
            })
        } else {
            data_205.usetou = even.currentTarget.name;
        }
