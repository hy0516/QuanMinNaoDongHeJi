...
import item_lianxian_225 from "./item_lianxian_225";
import lianxian_config_225 from "./lianxian_config_225";
import zc_lost225 from "./zc_lost225";
...
@ccclass
export default class lianxian_224 extends BaseGame {
    ...
    /**已连线节点数组 (用于线条重绘)*/
    public okList: item_lianxian_225[] = [];
    public maxlv = 5;
    public curlv = 1;

    public calledTipsAd: { [key: number]: boolean } = {};

    public restartLevel: number = 0;
    onLoad() {
        GameData.PauseGame = false;
        /**关卡初始化 */
        this.curlv = this.restartLevel || 1;
        this.initLevel();
    ...
    initLevel() {
        ...
        this.content.active = true; // 关键修复：激活当前关卡容器
        //更新标签
        this.labelUpdate();

        this.node.getChildByName(`btn_tips`).getChildByName(`luxiang`).active = true;

        //重置数组
        this.okList = [];
        this.curStartItem = null;

        var info = lianxian_config_225["level" + this.curlv.toString()];
        for (let i = 0; i < info.mapdata.length; i++) {
            for (let j = 0; j < info.mapdata[i].length; j++) {
                var data = info.mapdata[i][j];
                var item = cc.instantiate(this.item);
                item.getComponent(item_lianxian_225).oninit(data);
        ...
        if (!this.curStartItem || this.okList.length == 1) {
            ...
            for (let i = 0; i < childlist.length; i++) {
                ...
                if (childitem.getBoundingBox().contains(this.content.convertToNodeSpaceAR(event.getLocation()))) {
                    var thing = childitem.getComponent(item_lianxian_225);
                    //点击过的节点，跳过
    ...
    onTouchEnd() {
        ...
        if (this.okList.length == 1) {
            var thing = this.okList[0].getComponent(item_lianxian_225);
       ...
       // console.log(childlist);
        ...
        if (isWin) {
            ...
            var fun = () => {
                this.cha.scale = 0;
                GameData.PauseGame = false
                const lostNode = this.node.getChildByName("zc_lostend"); // 失败节点与当前节点同父级
                lostNode.active = true;
                const lostScript = lostNode.getComponent(zc_lost225);
                if (lostScript) {
                    lostScript.failedLevel = this.curlv; // 关键：记录失败的关卡
                }
            }
    ...
    BtnHandler(even: cc.Event.EventTouch) {
        ...
        switch (even.currentTarget.name) {
            ...
            case "btn_tijiao":
                ...
                break;
            case "btn_tips":
                if(this.calledTipsAd[this.curlv]) {
                    ...
                    var handlers = () => {
                        this.calledTipsAd[this.curlv] = true;
