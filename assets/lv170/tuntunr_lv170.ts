...
@ccclass
export default class tuntunr_lv161 extends BaseGame {
    @property(cc.Node)
    ...
    onLoad() {
        ...
        cc.tween(this.tittle)
            ...
            .start()
        AudioManager.playMusic("关卡背景_170", true);
    ...
    Initlevel() {
        ...
        if (this.role1.active) {
            this.role1.getChildByName("khx_ske").getComponent(dragonBones.ArmatureDisplay).playAnimation("bjx2", -1);
        }

        if (this.role2.active) {
            this.role2.getChildByName("kulh_ske").getComponent(dragonBones.ArmatureDisplay).playAnimation("nksy2", -1);
        }

        if (this.role3.active) {
            this.role3.getChildByName("fkl_ske").getComponent(dragonBones.ArmatureDisplay).playAnimation("mtc2", -1);
        }
    ...
    onTouchEnd(e: cc.Event.EventTouch) {
        ...
        if (isrun) {
            ...
            if (this.role1.active) {
                this.role1.getChildByName("khx_ske").getComponent(dragonBones.ArmatureDisplay).playAnimation("bjx", -1);
            }

            if (this.role2.active) {
                this.role2.getChildByName("kulh_ske").getComponent(dragonBones.ArmatureDisplay).playAnimation("nksy", -1);
            }

            if (this.role3.active) {
                this.role3.getChildByName("fkl_ske").getComponent(dragonBones.ArmatureDisplay).playAnimation("mtc", -1);
            }
    ...
    update(dt: number): void {
        ...
        if (time - this.curTime1 >= 25) {
            ...
            if (this.i < this.line1.length) {
                cc.tween(this.role1)
                    .to(0.5, { x: this.line1[this.i].x, y: this.line1[this.i].y })
                    .call(() => {
            ...
            if (this.k < this.line3.length) {
                cc.tween(this.role3)
                    .to(0.5, { x: this.line3[this.k].x, y: this.line3[this.k].y })
                    .call(() => {
                        this.k++;
            ...
            if (this.j < this.line2.length) {
                cc.tween(this.role2)
                    .to(0.5, { x: this.line2[this.j].x, y: this.line2[this.j].y })
                    .call(() => {
                        this.j++;
