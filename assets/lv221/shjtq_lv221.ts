...
import shjtq_lv221_move from "./shjtq_lv221_move";
...
@ccclass
export default class shjtq_lv220 extends BaseGame {
   ...
   // canMove:boolean=true;
       ...
       // this.unschedule(this.Timeing);
        ...
        for (let index = 0; index < enemNode.childrenCount; index++) {
            ...
            if(Math.abs(element.x+element.children[0].x)<30)
            {
               ...
               element.getComponent(shjtq_lv221_move).canwalk=false;
                this.bgNode.getChildByName(`tq_ske`).active=false;
                element.getComponent(dragonBones.ArmatureDisplay).timeScale=0.8;
                element.getComponent(dragonBones.ArmatureDisplay).playAnimation(`${element.name}`+ 'tq',1);
                AudioManager.playEffect(`lache_211`);
                this.addOneTimeListener(element.getComponent(dragonBones.ArmatureDisplay),()=>{

                   
                    if(element.name==`1`&&!this.isblxx)
                    {
                        //element.y=200;
                        element.getComponent(dragonBones.ArmatureDisplay).playAnimation(`1da`,1);
                        this.scheduleOnce(()=>{
                            AudioManager.playEffect(`blxxqj211`);
                        },0.8);
                        ...
                        this.addOneTimeListener(element.getComponent(dragonBones.ArmatureDisplay),()=>{
                         ...
                         //   this.canMove=true;
                            ...
                            element.getComponent(dragonBones.ArmatureDisplay).playAnimation(`1dj`,0);
                            element.getComponent(shjtq_lv221_move).canwalk=true;
                            element.x= element.getComponent(shjtq_lv221_move).tempX;
                    ...
                    }else
                    {
                        ...
                        .playAnimation(element.name+ 'dj',-1);
                        cc.tween(this.bgNode.getChildByName(`showSk`).getChildByName(`tq_ske`)).to(1,{scale:1}).call(()=>{
                            ...
                            AudioManager.playEffect(`sztc_211`);
                        }).start();
                        this.catchCount++;
        ...
        AudioManager.playEffect(`taokong_211`);
        ...
        for (let index = 0; index < enemNode.childrenCount; index++) {
            ...
            else
            {
                if(this.catchCount==3)
                {
                    ...
                    AudioManager.playEffect(`ndbs_211`);
    ...
    {
        AudioManager.playEffect(`${name}`+ "_lv211");
        switch (name) {
            case `1`:
            return `50000000`;
            case `2`:
