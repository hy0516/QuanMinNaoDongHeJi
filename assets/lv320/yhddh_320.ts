...
export default class shjddh_lv124 extends BaseGame {
    ...
    selNodeIndex = 0;
    dwSke: dragonBones.ArmatureDisplay = null;

    // ipadvideopath = [
    //     "https://cosartresources.zywxgames.com/users/yj/wgh/25_11/tt/mp4/xm.mp4",
    //     "https://cosartresources.zywxgames.com/users/yj/wgh/25_11/tt/mp4/hl.mp4",
    //     "https://cosartresources.zywxgames.com/users/yj/wgh/25_11/tt/mp4/jy.mp4",
    //     "https://cosartresources.zywxgames.com/users/yj/wgh/25_11/tt/mp4/bf.mp4",
    //     "https://cosartresources.zywxgames.com/users/yj/wgh/25_11/tt/mp4/zr.mp4",
    ...
    jietong() {
        ...
        if (cc.view.getVisibleSize().width >= 900) {
            if (this.dhNode.getChildByName("sknode").getChildByName(`${this.curselNode.children[0].name}_ske`).children[0]) {
                this.dwSke =this.dhNode.getChildByName("sknode").getChildByName(`${this.curselNode.children[0].name}_ske`).children[0].getComponent(dragonBones.ArmatureDisplay);
            }
            else {
                this.dwSke =this.dhNode.getChildByName("sknode").getChildByName(`${this.curselNode.children[0].name}_ske`).getComponent(dragonBones.ArmatureDisplay);
            }
            
            this.dwSke.playAnimation(`${this.curselNode.children[0].name}_pb`,0);
            if (this.curselNode.children[0].name == "amgdd") {
                this.dhNode.getChildByName("sknode").getChildByName(`${this.curselNode.children[0].name}_ske`).scale = 3;
            }
            // this.dhNode.getChildByName("sknode").getChildByName(`${this.curselNode.children[0].name}_ske`).scale = 1;
        }
        this.dhNode.getChildByName(`time`).getComponent(Timer320).resetTimer();
        this.dhNode.getChildByName(`time`).getComponent(Timer320).startTimer();
    }
