import Sheep_CorrallingFlock356 from "./Sheep_CorrallingFlock356";
import Wolf_CorrallingFlock356 from "./Wolf_CorrallingFlock356";

import Shepherd_CorrallingFlock356 from "./Shepherd_CorrallingFlock356";
import GameViewLogic_CorallingFlock356 from "./GameViewLogic_CorrallingFlock356";
import AnswerLogic_CorrallingFlock356 from "./AnswerLogic_CorrallingFlock356";
import CircleNode_CorrallingFlock356 from "./CircleNode_CorrallingFlock356";
import DX_Config356 from "./DX_Config356";
import DX_EventType356 from "./DX_EventType356";
import GameData from "../script/common/GameData";
import AudioManager from "../script/common/AudioManager";
import BaseGame from "../script/common/BaseGame";
import VideoManager from "../script/common/VideoManager";
import RouteLogic_CorrallingFlock356 from "./RouteLogic_CorrallingFlock356";
...
@ccclass
export default class LevelCtrl_CorrallingFlock356  extends BaseGame {
    ...
    private gameViewLogic: GameViewLogic_CorallingFlock356 = null;
    private sheepArr: Sheep_CorrallingFlock356[] = null;
    private wolfArr: Wolf_CorrallingFlock356[] = null;
    ...
    onLoad() {
        ...
        this.shepherd = this.map.getComponentInChildren(Shepherd_CorrallingFlock356).node;
        ...
        this.gameViewLogic = this.map.getChildByName("GameViewLogic").getComponent(GameViewLogic_CorallingFlock356);
        this.sheepArr = this.map.getComponentsInChildren(Sheep_CorrallingFlock356);
        this.wolfArr = this.map.getComponentsInChildren(Wolf_CorrallingFlock356);
    ...
    start() {
        ...
        this.node.on(DX_EventType356.CorrallingFlock_OnDrawFinish, this.onDrawFinish, this);


        // 
        this.node.on(DX_EventType356.CrossTheRiver_OnGameFail, this.onGameFailEvent, this);

        // if(this.gameViewLogic.getComponentInChildren(AnswerLogic_CorrallingFlock356)){
            // this.gameViewLogic.getComponentInChildren(AnswerLogic_CorrallingFlock356).onDrawAnswer(DX_Config356.CorrallingFlock_Guide);
        // }
    }
    onDestroy() {
        this.node.off(DX_EventType356.CorrallingFlock_OnDrawFinish, this.onDrawFinish, this);
        this.node.off(DX_EventType356.CrossTheRiver_OnGameFail, this.onGameFailEvent, this);
    ...
    onDrawFinish(event: cc.Event.EventCustom) {
        ...
        if(this.gameViewLogic.getComponentInChildren(AnswerLogic_CorrallingFlock356)){
            this.gameViewLogic.getComponentInChildren(AnswerLogic_CorrallingFlock356).node.destroy();
        }
    }
    ...
    update() {
        if (this.m_isDrawFinish) {
            ...
            for (let i = 0; i < this.circleArr.length; i++) {
                ...
                this.circleArr[i].getComponent(CircleNode_CorrallingFlock356).onShorted(startPos, endPos);
            }
        ...
        while (worldPoints.length > 0) {
            ...
            while (nextPos != currPos) {
                ...
                if (sub.mag() >= this.m_circleDiameter) {
                    ...
                    circle.getComponent(CircleNode_CorrallingFlock356).setShepherd(this.shepherd);
                    // 设置距离关节
                    if (lastCircle) {
