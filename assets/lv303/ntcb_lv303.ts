...
import moveItems303 from "./moveItems303";

const { ccclass, property } = cc._decorator;

@ccclass
export default class ntcb_lv303 extends BaseGame {
    @property(cc.Node)
    p_ske: cc.Node = null;
    @property(cc.Node)
    eyes: cc.Node = null;
    ...
    anime: string[] = [`cdf`,`cdf`,`kl`,`hb`,`hjm`,`dwx`,`ll`,`dc`,`lt`,`kc`];
    ...
    onLoad() {
        ...
        this.scheduleOnce(() => {
            AudioManager.playMusic(`bgmlv303`);
        }, 0.5)
    ...
    moveHandler(type: number, tar: cc.Node, even: cc.Event.EventTouch) {
        ...
        if (checkNode && checkNode.active && cc.Intersection.rectRect(checkNode.getBoundingBox(), itemNode.getBoundingBox())) {
            ...
            } else if (type !==2 ||type ===2 && itemNode.getComponent("moveItems303").isShaked === false) {
                AudioManager.playEffect(`${this.music[type]}`);
                this.animeSingle(this.psk, `lm_${this.anime[type]}`);
                this._delayTotal = this.time[type];
                this.sum++;
        ...
        // 非碰撞场景
        if (itemNode.active) {
            itemNode.getComponent(moveItems303)?.restart();
    ...
    eyeslookat(ccp: cc.Vec3) {
        this.eyes.getComponent("eyes_lv303").eyeslookat(ccp);
    }
