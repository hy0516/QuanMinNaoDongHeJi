...
export interface ILv386MainForLostPanel {
...
@ccclass
export default class lostPanel_lv386 extends cc.Component {

    /** 主游戏引用（fuhuo 时调用 lvUpdate） */
    public mainGame: ILv386MainForLostPanel = null;
    ...
    onClose() {
        ...
        cc.resources.load("prefabs/common/Hall", cc.Prefab, (err, hall: cc.Prefab) => {
            if (err || !hall) {
                cc.error("[lostPanel_lv386] 加载 Hall 失败", err?.message);
                return;
            }
    ...
    onFuhuo() {
        ...
        VideoManager.getInstance().showVideo(() => {
            ...
            console.log("[lostPanel_lv386] 广告调用失败");
        });
    }
    ...
    onRestart() {
        ...
        AssetManager.load(GameData.curGameStyle, GameData.curGameName, cc.Prefab, null, (prefab: cc.Prefab) => {
            if (!prefab) {
                cc.error("[lostPanel_lv386] 加载关卡失败");
                return;
            }
