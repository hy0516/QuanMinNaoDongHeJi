...
export default class PlatformAdManager {
    ...
    */
    ...
    /**
     * ⚠️⚠️设置Feed流进入/退出小游戏事件回调(需要在feedPlanExecute之前完成设置)
     * @param feedEnterCallback Feed流进入小游戏事件回调
     * @param feedExitCallback Feed流退出小游戏事件回调
     */
    setFeedStateChangeCallBack(feedEnterCallback?: Function, feedExitCallback?: Function) {
        switch (AdsOptions.platform) {
            case LPPlatform.toutiao:
                PlatformAdManager.dxPFGameAPI.setFeedStateChangeCallBack(feedEnterCallback, feedExitCallback);
                break;
            default:
                break;
        }
