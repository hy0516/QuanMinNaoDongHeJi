
import AudioManager from "../script/common/AudioManager";
import tipsPanel from "../script/zc/tipsPanel";
import jzhzppwg_lv1 from "./jzhzppwg_lv1";





const { ccclass, property } = cc._decorator;
enum music {
    开门声 = "开门声",
    开窗声 = "开窗声",
}

@ccclass
export default class jzhzppwg_lv1_move extends cc.Component {

    @property(cc.String)
    type: string = "";
    @property(cc.Node)
    target: cc.Node = null;
    @property(cc.Node)
    mainNode: cc.Node = null;
    startPoi: cc.Vec2 = null;
    copyMove: cc.Vec2 = new cc.Vec2(0, 0);
    RZindex: number
    state:number=0;//1是已匹配


    public originalParent: cc.Node = null;
    public originalIndex: number = 0;
    
    protected onLoad(): void {
        this.startPoi = this.node.getPosition();
        this.enabled = false;
        this.node.on(cc.Node.EventType.TOUCH_START, this.onTouchStart, this);
        this.node.on(cc.Node.EventType.TOUCH_MOVE, this.onTouchMove, this);
        this.node.on(cc.Node.EventType.TOUCH_END, this.onTouchEnd, this);
        this.node.on(cc.Node.EventType.TOUCH_CANCEL, this.onTouchEnd, this);
       // this.target=this.mainNode.getComponent(nzxfx_lv1).allrole;

       // 记录原始状态
       this.originalParent = this.node.parent;
       this.originalIndex = this.node.getSiblingIndex();
    }
    onTouchStart(even: cc.Event.EventTouch) {
        if(this.mainNode.getComponent(jzhzppwg_lv1).health<=0)return;
       if(!this.mainNode.getComponent(jzhzppwg_lv1).iscanmove)return;
       
    //     // if (this.isChangeIndex == 0)
    //     // this.node.zIndex = 100;
    //    this.RZindex = this.node.zIndex;
    //     this.node.zIndex = 50;
   
         this.copyMove = this.node.parent.convertToWorldSpaceAR(this.node.getPosition());//new cc.Vec2(0, 0); 
          
          
          // 移动到根节点并置顶
          this.node.parent =cc.director.getScene();
          this.node.setSiblingIndex(cc.macro.MAX_ZINDEX);

          this.node.x = this.copyMove.x;
          this.node.y = this.copyMove.y;
    }
    onTouchMove(even: cc.Event.EventTouch) {
        if(this.mainNode.getComponent(jzhzppwg_lv1).health<=0)return;
       if(!this.mainNode.getComponent(jzhzppwg_lv1).iscanmove)return;
        var delay = even.getDelta();
        this.copyMove.x += delay.x;
        this.copyMove.y += delay.y;
        this.node.x = this.copyMove.x;
        this.node.y = this.copyMove.y;
        
    }
    onTouchEnd(even: cc.Event.EventTouch) {
        if(this.mainNode.getComponent(jzhzppwg_lv1).health<=0)return;
        if(!this.mainNode.getComponent(jzhzppwg_lv1).iscanmove)return;
        this.mainNode.getComponent(jzhzppwg_lv1).iscanmove=false;
        var ishuan =false;
        for (let index = 0; index < this.mainNode.getComponent(jzhzppwg_lv1).clicklist.childrenCount; index++) {
            const element = this.mainNode.getComponent(jzhzppwg_lv1).clicklist.children[index];
            element.getChildByName(`quan`).active=false;
        }
        for (let index = 0; index < this.mainNode.getComponent(jzhzppwg_lv1).clicklist.childrenCount; index++) {
            const element = this.mainNode.getComponent(jzhzppwg_lv1).clicklist.children[index];
            element.getChildByName(`quan`).active=false;
            const nodeofface =element.getChildByName(`facepoint`).children[0];
           if(nodeofface!=undefined&&nodeofface.getComponent(jzhzppwg_lv1_move).state==1)continue;
           if(nodeofface===this.node)continue;
            if(nodeofface!=undefined&&cc.Intersection.rectRect(nodeofface.getBoundingBoxToWorld(),this.node.getBoundingBoxToWorld()))//获取节点的世界坐标
            {
                ishuan=true;
                //有相交的情况  切换
                this.restart();
                this.scheduleOnce(()=>{
                    this.mainNode.getComponent(jzhzppwg_lv1).swapNodesPosition(nodeofface,this.node);
                },0.2)
                break;
            }else
            {
                continue
            }
            
        }
       if(!ishuan) this.restartAndmove();
       
    }
    restartAndmove() {
        // 恢复原始父节点和层级
        this.node.parent = this.originalParent;
        this.node.setSiblingIndex(this.originalIndex);
        this.node.setPosition(this.startPoi);
        this.scheduleOnce(()=>{
            this.mainNode.getComponent(jzhzppwg_lv1).iscanmove=true;
        },0.2)
        
    }
    restart() {
        // 恢复原始父节点和层级
        this.node.parent = this.originalParent;
        this.node.setSiblingIndex(this.originalIndex);
        this.node.setPosition(this.startPoi);
    }
    ondele() {
        this.node.off(cc.Node.EventType.TOUCH_START, this.onTouchStart, this);
        this.node.off(cc.Node.EventType.TOUCH_MOVE, this.onTouchMove, this);
        this.node.off(cc.Node.EventType.TOUCH_END, this.onTouchEnd, this);
        this.node.off(cc.Node.EventType.TOUCH_CANCEL, this.onTouchEnd, this);
        this.state=1;
    }

}

