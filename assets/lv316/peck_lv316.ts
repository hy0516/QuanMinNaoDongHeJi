...
@ccclass
export default class peck_lv316 extends cc.Component {
    ...
    onCollisionEnter(other: cc.Collider, self: cc.Collider) {
        if (other.tag === 4) {
            ...
            other.node.getComponent(`bug_lv316`)?.stopTagSwitch();
            this.isdead = true;
        } else if (other.tag === 1) {
            ...
            other.node.getComponent(`boom_lv316`).boom();
            other.tag = 0;
            other.node.opacity = 0;
        } else if (other.tag === 3) {
            ...
            other.tag = 0;
            other.node.opacity = 0;
            other.node.getComponent(`ice_lv316`).ice();
        } else if (other.tag === 5) {
            AudioManager.playEffect("电击");
            other.node.getComponent(`light_lv316`).light();
