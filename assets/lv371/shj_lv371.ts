...
import item_move_lv371 from "./item_move_lv371";
const { ccclass, property } = cc._decorator;

@ccclass
export default class shj_lv371 extends BaseGame {
    @property(cc.Node)
    ...
    tipsGroupFilter(puzzNode:cc.Node){
        ...
        // 停止目标节点的闪烁动画
        let targetNode = puzzNode.getComponent(item_move_lv371).targetNode;
        if (targetNode) {
    ...
    tipEffect(){
        ...
        let target = puzz.getComponent(item_move_lv371).targetNode;

        //若状态不正确,反转
        if(puzz.getComponent(item_move_lv371).stateCreccot == false){
            puzz.getComponent(item_move_lv371).puzzTurn();
        }
        target.active = true;
        target.opacity = 255;
        puzz.opacity = 255;
    ...
    puzzInof(){
        ...
        for(let i = 0; i < puzzs.length; i++ ){
            let puzz = puzzs[i];
            puzz.addComponent(item_move_lv371);
            puzz.getComponent(item_move_lv371).puzzInof(this.node);
            ...
            if(name.slice(-2) == "jx"){
                puzz.scaleX = -1;
                //默认翻转状态为 false
                puzz.getComponent(item_move_lv371).stateCreccot = false;
            }
    ...
    getPuzzTargetNode(puzzNode:cc.Node){
        ...
        //let puzzTarget = puzzNode.getComponent(item_move_lv292).targetNode;
        ...
        for(let i = 0 ; i < skins.length ; i++ ){
            ...
            if(pointNode.name == puzzNode.name){
                ...
                puzzNode.getComponent(item_move_lv371).targetNode = pointNode;
               // console.log(puzzNode.name,puzzNode.getComponent(item_move_lv371).targetNode);
            }
        }
    }
