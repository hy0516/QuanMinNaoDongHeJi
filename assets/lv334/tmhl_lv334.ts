...
@ccclass
export default class tmhl_lv334 extends BaseGame {
    ...
    protected onLoad(): void {
        ...
        this.scheduleOnce(() => {
            AudioManager.playMusic("关卡背景lv334", false, 0.5);
        }, 0.5);


        this.time.string = "时间:" + this.startTime.toString() + "s";
        this.schedule(this.Timeing.bind(this), 1);
    ...
    gameInit() {
        for (let i = 1; i <= 8; i++) {
            this.gameConfig[i].value = this.lvNodes.children[i-1].children[0].children.length;
        }
    }
    ...
    upAndDownPage(isRight: boolean) {
        ...
        if (this.pageView.getCurrentPageIndex() + uppage == 8){
            AudioManager.playEffect("切页");
            ...
            AudioManager.playEffect("切页");
            this.pageView.scrollToPage(7, 0.5);
            // this.curLv = 5;
        }
        ...
        if (this.lvNum[this.curLv - 1] == this.gameConfig[this.curLv].value) {
            ...
            cc.tween(self.bigGou)
                ...
                .call(() => {
                    ...
                    else if (self.curLv == 8 && self.winValue < 7) {
                        self.curLv = 1;
                        // AudioManager.playEffect("切页");
    ...
    showTisp(){
        ...
        let quan2;
        let toolName;
        for (let i = 0; i < this.lvNodes.children[this.curLv - 1].children[0].children.length; i++){
            if (!this.lvNodes.children[this.curLv - 1].children[0].children[i].getChildByName("tb").active) {
                toolName = this.lvNodes.children[this.curLv - 1].children[0].children[i].name;
            }
            
            for (let j = 0; j < this.toolNodes.children.length; j++){
                if (this.toolNodes.children[j]!= null && toolName == this.toolNodes.children[j].children[0].name) {
                    ...
                    if (!cc.isValid(this.q2Node) || this.q2Node == null) {
                        ...
                        this.lvNodes.children[this.curLv - 1].children[0].children[i].addChild(quan2);
                        this.q2Node = quan2;
                    }
