...
import ddl_lv207 from "./ddl_lv207";
import player_lv207 from "./player_lv207";
const { ccclass, property } = cc._decorator;

@ccclass
export default class qsh_lv207 extends BaseGame {
    @property(cc.Node)
    ...
    movespeed = 9;
    onLoad() {
      this.mainTs=cc.find("Canvas/" + GameData.curGameName).getComponent(ddl_lv207);
       // console.log((`qsh`));
      // this.node.scale=0.8;
    }
    ...
    collCheck()
    {
        if(cc.Intersection.rectRect(this.node.children[0].getBoundingBoxToWorld(),this.mainTs.player.getChildByName(`collNode`).getBoundingBoxToWorld()))
        {
            ...
            this.mainTs.player.getComponent(player_lv207).interruptJump();
