...
import shj_lv274 from "./shj_lv274";


const { ccclass, property } = cc._decorator;

@ccclass
export default class itme_move_lv274 extends cc.Component {
 ...
 // @property(cc.Integer)
   ...
   startIndex = 0;
    ...
    puzzInof(main:cc.Node){
        ...
        //获取目标节点
        this.mainNode.getComponent(shj_lv274).getPuzzTargetNode(this.node);
    ...
    onTouchMove(event: cc.Event.EventTouch) {
        ...
        /*if(this.posInof(event)){
            ...
            //不在提示状态中，则关闭
            if(this.mainNode.getComponent(shj_lv274).tipLock ==false){
                this.targetNode.active = false;
            }   
    ...
    onTouchEnd(event: cc.Event.EventTouch) {
        ...
        if(this.posInof(event)){
            ...
            //拼图完成+1
            this.mainNode.getComponent(shj_lv274).curPuzz_num += 1;

            this.mainNode.getComponent(shj_lv274).updatelv();


            //播放音效
            AudioManager.playEffect("right274");
           
            //关闭闪烁
            this.mainNode.getComponent(shj_lv274).tipLock = false;
            //剔除当前节点
            this.mainNode.getComponent(shj_lv274).tipsGroupFilter(this.node);

            //提示开锁
            this.mainNode.getComponent(shj_lv274).lock_clickDouble = true;
