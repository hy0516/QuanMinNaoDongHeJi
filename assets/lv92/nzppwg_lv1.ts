...
import nzppwg_lv1_move from "./nzppwg_lv1_move";
import nzppwg_lv1_role from "./nzppwg_lv1_role";
...
@ccclass
export default class nzppwg_lv1 extends BaseGame {
   ...
   rightnummax=12;
    ...
    onLoad() {
        ...
        AudioManager.playMusic(music.关卡背景, true, 0.9);
   ...
   swapNodesPosition(nodeA: cc.Node, nodeB: cc.Node) {
    ...
    checkisright(nodeA: cc.Node, nodeB: cc.Node)
    {
        ...
        if(nodeA.parent.parent===nodeA.getComponent(nzppwg_lv1_move).target)
        {
            a=true;
            nodeA.getComponent(nzppwg_lv1_move).ondele();
            nodeA.parent.parent.getChildByName(`dui`).active=true;
            this.rightnum++;
        }
        if(nodeB.parent.parent===nodeB.getComponent(nzppwg_lv1_move).target)
        {
            b=true;
            nodeB.getComponent(nzppwg_lv1_move).ondele();
                ...
                VideoManager.getInstance().showVideo(() => {
                    for (let index = 0; index < this.clicklist.childrenCount; index++) {
                        ...
                        else
                        {
                            ...
                            element.getComponent(nzppwg_lv1_role).target.parent.parent.getChildByName(`quan`).active=true;
                        }
                        break;
                    }
                })
