...
@ccclass
export default class hz_lv12 extends BaseGame {
    ...
    moveHandler(type: number, tar: cc.Node, even: cc.Event.EventTouch): void {
        ...
        if (type == 2) {
            if (this.yg.active && this.yg.getBoundingBox().contains(poi)) {
                even.currentTarget.active = false;
                this.yg.active = false;
                this.lian.active = true;
        ...
        if (type == 3) {
            if (this.lian.active && this.lian.getBoundingBox().contains(poi)) {
                even.currentTarget.destroy();
                this.lian.active = false;
                this.nv3.active = true;
                var yansk = this.node.getChildByName("bg").getChildByName("yan_sk");
                // @ts-ignore
                yansk.position = poi
                yansk.active = true;
        ...
        for (let i = 0; i < this._itemList.length; i++) {
            ...
            if (item.getBoundingBox().contains(poi)) {
                if (item.name == "nv") {
                    this.showPB("我不需要钱");
                    AudioManager.playEffect("hz12_1");
                    even.currentTarget.getComponent(moveItems2).restart();
                    return
                }
                if (item.name == "zhong") {
                    this.shalou.active = true;
                }
                if (item.name == "tong") {
                    this.yg.active = true;
                }
