...
import item_lianxian_237 from "./item_lianxian_237";
import lianxian_config_237, { data_237 } from "./lianxian_config_237";
...
@ccclass
export default class lianxian_237 extends BaseGame {
    ...
    /**已连线节点数组 (用于线条重绘)*/
    public okList: item_lianxian_237[] = [];
    public maxlv = 5;
    public curlv = 1;
    ...
    onLoad() {
        ...
        console.log("curlv:",data_237.curlv);
        ...
        this.scheduleOnce(()=>{
            AudioManager.playMusic("bgm_237");
        },0.5);
    ...
    initLevel() {
        ...
        this.content = this.contentList[data_237.curlv - 1];
        //更新标签
        this.labelUpdate();
        ...
        var info = lianxian_config_237["level" + data_237.curlv.toString()];

        for (let i = 0; i < info.mapdata.length; i++) {
            for (let j = 0; j < info.mapdata[i].length; j++) {
                var data = info.mapdata[i][j];
                var item = cc.instantiate(this.item);
                item.getComponent(item_lianxian_237).oninit(data);
                if(data_237.curlv == 1) {
                    item.scale = 0.7;
                    item.children[0].setAnchorPoint(0.5,0.8);
                    item.children[1].setAnchorPoint(0.5,0.65);
                    item.setAnchorPoint(0.5,0.6);

                    item.children[1].getChildByName("ske").y = -118;
                } 
                if (data_237.curlv == 2) {
                    item.scale = 0.7;
                    item.children[0].setAnchorPoint(0.5,0.8);
                ...
                if (data_237.curlv == 4){
                    item.scale = 0.5;
                    item.children[0].setAnchorPoint(0.5,1.2);
                    item.children[1].setAnchorPoint(0.5,1);
        ...
        if (!this.curStartItem || this.okList.length == 1) {
            ...
            for (let i = 0; i < childlist.length; i++) {
                ...
                if (childitem.getBoundingBox().contains(this.content.convertToNodeSpaceAR(event.getLocation()))) {
                    var thing = childitem.getComponent(item_lianxian_237);
                    //点击过的节点，跳过
    ...
    onTouchEnd() {
        ...
        if (this.okList.length == 1) {
            var thing = this.okList[0].getComponent(item_lianxian_237);
       ...
       // console.log(childlist);
        ...
        if (isWin) {
            ...
            var fun = () => {
                ...
                let lostPanel = cc.instantiate(this.lostPanel);
                this.node.parent.addChild(lostPanel);
                lostPanel.opacity = 0;
                cc.tween(lostPanel)
                .to(0.8, { opacity: 255 })
                .start()
        ...
        switch (even.currentTarget.name) {
            ...
            case "btn_tips":
                var handlers = () => {
                    ...
                    if (data_237.curlv != 1) this.tipsPanel.getChildByName("tishi" + (data_237.curlv - 1).toString()).active = false;
                    this.tipsPanel.getChildByName("tishi" + (data_237.curlv).toString()).active = true;
                ...
                break;
            case "x":
                this.tipsPanel.getChildByName("tishi" + (data_237.curlv).toString()).active = false;
    ...
    labelUpdate(){
        this.Lable.string = " " + data_237.curlv + " " + "/" + " 5" ;
    }
    loadendlost() {
