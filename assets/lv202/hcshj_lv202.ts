...
import HCSHJ_EventType from "./HCSHJ_EventType_202";
...
private processingPairs: Set<string> = new Set();
    onLoad() {
        ...
        this.node.on(HCSHJ_EventType.CONTACTCOLLIDER_EVENT,this.onContactBegin,this)
        this.node.on(HCSHJ_EventType.GAMELOST_EVENT,this.onlost,this)
    ...
    UpdateNewNode()
    {
        ...
        this.curNewNode.setPosition( this.node.getChildByName(`bg`).children[0].getChildByName(`startNode`).position.clone());
    ...
    private mergeNodes(nodeA: cc.Node, nodeB: cc.Node,nocenterPos?:boolean) {
        ...
        let newPre =this.getHeChengPrefab(nodeA.name)
         ...
         AudioManager.playEffect(`${nodeA.name}`);//播放合成声音
        // 立即销毁原节点（重要！）
        nodeA.destroy();
        nodeB.destroy();
        if(newPre==null)
