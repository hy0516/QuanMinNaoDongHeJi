...
@ccclass
export default class zc_lv3 extends BaseGame {

    @property(cc.Label)
    humanNum: cc.Label = null;
    ...
    updateGameNum(num: number) {
        if (num == 0) {
            this.humanNum.string = this.curNum.toString() + "/" + this.totalNum.toString() + "人";
            return
        }
        this.addhumantips.getComponent(cc.Label).string = "+" + num.toString();
        cc.Tween.stopAllByTarget(this.addhumantips);
        cc.tween(this.addhumantips)
            .to(0.2, { opacity: 255 })
            .delay(0.5)
            .to(0.1, { opacity: 0 })
            .call(() => {
            ...
            case "tishi":
                var handlers = () => {
                    cc.resources.load("prefabs/zc/TipPanel", cc.Prefab, (err, tip: cc.Prefab) => {
                        ...
                        HallnNode.getComponent(tipsPanel).curtipList = zc_config.lv3tip;
                        HallnNode.parent = cc.find("Canvas");
    ...
    tweenState2(event: cc.Event.EventTouch, tar: cc.Node, name: string, handler?: Function, audio?: string) {
        ...
        handler && handler();
        var url = zc_config.lv3[name].split("|");
        cc.tween(this.icon)
