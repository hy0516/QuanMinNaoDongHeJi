...
@ccclass
export default class zc_lv4 extends BaseGame {
            ...
            case "tishi":
                var handlers = () => {
                    cc.resources.load("prefabs/zc/TipPanel", cc.Prefab, (err, tip: cc.Prefab) => {
                        ...
                        HallnNode.getComponent(tipsPanel).curtipList = zc_config.lv4tip;
                        HallnNode.parent = cc.find("Canvas");
    ...
    tweenState2(event: cc.Event.EventTouch, tar: cc.Node, name: string, handler?: Function, audio?: string) {
        ...
        // let endnodepo = this.view.convertToWorldSpaceAR(endnode.position);
        // let ebdnodepo2 = this.node.convertToNodeSpaceAR(endnodepo)
        //@ts-ignore
        ...
        handler && handler();
        var url = zc_config.lv4[name].split("|");
        cc.tween(this.icon)
            .delay(0.4)
