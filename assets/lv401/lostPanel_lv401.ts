...
/** lv401 失败面板与主关卡仅保留 node 引用（用于销毁） */
export interface ILv401MainForLostPanel {
    node: cc.Node;
}

@ccclass
export default class lostPanel_lv401 extends cc.Component {
    public mainGame: ILv401MainForLostPanel = null;
    ...
    onClose() {
        ...
        cc.resources.load("prefabs/common/Hall", cc.Prefab, (err, hall: cc.Prefab) => {
            if (err || !hall) {
                cc.error("[lostPanel_lv401] 加载 Hall 失败", err?.message);
                return;
            }
    ...
    onWatchAdNextLevel() {
        ...
        VideoManager.getInstance().showVideo(() => {
            ...
            console.log("[lostPanel_lv401] 广告调用失败");
        });
    }
    ...
    onRestart() {
        ...
        AssetManager.load(GameData.curGameStyle, GameData.curGameName, cc.Prefab, null, (prefab: cc.Prefab) => {
            if (!prefab) {
                cc.error("[lostPanel_lv401] 加载关卡失败");
                return;
            }
