...
import item_wz_455 from "./item_wz_455";
import wenzi_config_455 from "./wenzi_config_455";
...
@ccclass
export default class game_wenzi_455 extends BaseGame {
    ...
    onLoad() {
        ...
        for (let i = 0; i < wenzi_config_455.level1.mapdata.length; i++) {
            for (let j = 0; j < wenzi_config_455.level1.mapdata[i].length; j++) {
                var data = wenzi_config_455.level1.mapdata[i][j];
                var item = cc.instantiate(this.item);
                item.getComponent(item_wz_455).oninit(data);
                item.parent = this.content;
            }
        }
        GameData.startTime = 90;
        this.time.string = GameData.startTime.toString() + "s";
        this.schedule(this.Timeing, 1)
        // AudioManager.playMusic(AudioManager.wenzi.main_wz, true, 1);
        // this.scheduleOnce(() => {
        //     AudioManager.playMusic(`bgmlv455`);
        // }, 0.5);
    ...
    onTouchMove(event: cc.Event.EventTouch) {
        ...
        for (let i = 0; i < childlist.length; i++) {
            ...
            if (childitem.getBoundingBox().contains(this.content.convertToNodeSpaceAR(event.getLocation()))) {
                var thing = childitem.getComponent(item_wz_455);
    ...
    checkAnswerd(list: string) {
        if (wenzi_config_455.levelanswerd.indexOf(list) != -1) {
            GameData.wenziWinNum++;
            AudioManager.playEffect(wenzi_config_455.levelSound[this.curId - 1]);
            // console.log(GameData.wenziWinNum);
            if (GameData.wenziWinNum == wenzi_config_455.levelanswerd.length) {
                // console.log("赢了");
                ...
                //         .call(() => {
                        ...
                        // .to(0.5, { opacity: 150 })
                        // .start();
                    // AudioManager.playEffect(`彩蛋bgm`, false, () => {
                    //         this.onwin();
                    // });
                // }, 1)
            }
    ...
    cuowu() {
        ...
        this.iscuowu = true;
        AudioManager.playEffect(wenzi_config_455.lineLost);
        this.dacha();
        setTimeout(() => {
    ...
    Timeing() {
        ...
        if (GameData.startTime == 0) {
            ...
            this.unschedule(this.Timeing)
            this.scheduleOnce(() => {
                AssetManager.load(GameData.curGameStyle, "endlost_wz_455", cc.Prefab, null, (name: cc.Prefab) => {
