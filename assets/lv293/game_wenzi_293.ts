...
import item_wz_293 from "./item_wz_293";
import wenzi_config_293 from "./wenzi_config_293";
...
@ccclass
export default class game_wenzi_293 extends BaseGame {
    ...
    onLoad() {
        ...
        for (let i = 0; i < wenzi_config_293.level1.mapdata.length; i++) {
            for (let j = 0; j < wenzi_config_293.level1.mapdata[i].length; j++) {
                var data = wenzi_config_293.level1.mapdata[i][j];
                var item = cc.instantiate(this.item);
                item.getComponent(item_wz_293).oninit(data);
    ...
    onTouchMove(event: cc.Event.EventTouch) {
        ...
        for (let i = 0; i < childlist.length; i++) {
            ...
            if (childitem.getBoundingBox().contains(this.content.convertToNodeSpaceAR(event.getLocation()))) {
                var thing = childitem.getComponent(item_wz_293);
                if (thing.isset) continue;
                AudioManager.playEffect("键盘点击声_293");
    ...
    checkAnswerd(list: string) {
        if (wenzi_config_293.levelanswerd.indexOf(list) != -1) {
            GameData.wenziWinNum++;
            AudioManager.playEffect(wenzi_config_293.levelSound[this.curId - 1]);
            // console.log(GameData.wenziWinNum);
            if (GameData.wenziWinNum == wenzi_config_293.levelanswerd.length) {
                // console.log("赢了");
                ...
                this.scheduleOnce(()=>{
                    ...
                    .call(()=>{
                        ...
                        AudioManager.playEffect(`彩蛋_293`);
                        this.scheduleOnce(()=>{
                            this.onwin();
                        },12)
                    })
    ...
    cuowu() {
        ...
        if (this.iscuowu) return
        this.iscuowu = true;
        AudioManager.playEffect(wenzi_config_293.lineLost);
        this.dacha();
        setTimeout(() => {
    ...
    endAddTime() {
        AudioManager.playEffect(wenzi_config_293.button_wz);
        // GameData.PauseGame = false;
        // this.unschedule(this.Timeing);
    ...
    Timeing() {
        ...
        if (GameData.startTime == 0) {
            ...
            this.unschedule(this.Timeing)
            this.scheduleOnce(() => {
                AssetManager.load(GameData.curGameStyle, "endlost_wz_293", cc.Prefab, null, (name: cc.Prefab) => {
