...
import evenMap_235 from "./evenMap_235";
...
@ccclass
export default class moveitem_235 extends cc.Component {
    ...
    protected onLoad(): void {
        ...
        if (this.isBody) {
            this.node.on(evenMap_235.BodyIndex, () => {
                if (!this.isTouch) this.node.zIndex = 2;
            }, this)
        }
        this.node.on(evenMap_235.TouchCancel, () => {
            this.ondele();
        }, this)
    }
    ...
    onTouchStart(even: cc.Event.EventTouch) {
       ...
       if (!this.isBody)  this.node.opacity = 160;
        ...
        for (let i = 0; i < this.node.parent.childrenCount; i++) {
            ...
            chil.emit(evenMap_235.BodyIndex);
        }
        ...
        if (cc.Intersection.pointInPolygon(cc.v2(poi.x, poi.y), poly)) {
            ...
            cc.tween(this.node)
                ...
                .call(() => {
                    ...
                    if (this.di) this.di.active = false;
                    if (this.isHideChildren) this.node.children[0].active = true;
                    this.scheduleOnce(() => { this.node.dispatchEvent(new cc.Event.EventCustom(evenMap_235.CheckWuTai, true)); }, 0.010)
                })
            ...
            AudioManager.playEffect("爱心02_235");
            if (this.di) {
