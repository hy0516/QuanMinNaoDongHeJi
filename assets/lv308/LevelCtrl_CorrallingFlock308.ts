import Sheep_CorrallingFlock308 from "./Sheep_CorrallingFlock308";
import Wolf_CorrallingFlock308 from "./Wolf_CorrallingFlock308";

import Shepherd_CorrallingFlock308 from "./Shepherd_CorrallingFlock308";
import GameViewLogic_CorallingFlock308 from "./GameViewLogic_CorrallingFlock308";
import AnswerLogic_CorrallingFlock308 from "./AnswerLogic_CorrallingFlock308";
import CircleNode_CorrallingFlock308 from "./CircleNode_CorrallingFlock308";
import DX_Config308 from "./DX_Config308";
import DX_EventType308 from "./DX_EventType308";
import GameData from "../script/common/GameData";
import AudioManager from "../script/common/AudioManager";
import BaseGame from "../script/common/BaseGame";
import VideoManager from "../script/common/VideoManager";
import RouteLogic_CorrallingFlock308 from "./RouteLogic_CorrallingFlock308";
...
@ccclass
export default class LevelCtrl_CorrallingFlock308  extends BaseGame {
    ...
    pig_ske:dragonBones.ArmatureDisplay = null;
    ...
    private gameViewLogic: GameViewLogic_CorallingFlock308 = null;
    private sheepArr: Sheep_CorrallingFlock308[] = null;
    private wolfArr: Wolf_CorrallingFlock308[] = null;
    ...
    onLoad() {
        ...
        AudioManager.playMusic(music.关卡背景lv296, true, 0.6);

        
        this.map = this.node.getChildByName("map");
        this.shepherd = this.map.getComponentInChildren(Shepherd_CorrallingFlock308).node;
        ...
        this.gameViewLogic = this.map.getChildByName("GameViewLogic").getComponent(GameViewLogic_CorallingFlock308);
        this.sheepArr = this.map.getComponentsInChildren(Sheep_CorrallingFlock308);
        this.wolfArr = this.map.getComponentsInChildren(Wolf_CorrallingFlock308);
    ...
    start() {
        ...
        this.node.on(DX_EventType308.CorrallingFlock_OnDrawFinish, this.onDrawFinish, this);


        // 
        this.node.on(DX_EventType308.CrossTheRiver_OnGameFail, this.onGameFailEvent, this);

        if(this.gameViewLogic.getComponentInChildren(AnswerLogic_CorrallingFlock308)){
            this.gameViewLogic.getComponentInChildren(AnswerLogic_CorrallingFlock308).onDrawAnswer(DX_Config308.CorrallingFlock_Guide);
        }
    }
    onDestroy() {
        this.node.off(DX_EventType308.CorrallingFlock_OnDrawFinish, this.onDrawFinish, this);
        this.node.off(DX_EventType308.CrossTheRiver_OnGameFail, this.onGameFailEvent, this);
    ...
    onDrawFinish(event: cc.Event.EventCustom) {
        ...
        this.pig_ske.playAnimation("la", 1);
        this.scheduleOnce(() => {
            this.pig_ske.playAnimation("dj", 0);
        }, 1)
        ...
        if(this.gameViewLogic.getComponentInChildren(AnswerLogic_CorrallingFlock308)){
            this.gameViewLogic.getComponentInChildren(AnswerLogic_CorrallingFlock308).node.destroy();
        }
    }
    ...
    CheckGameResult() {
        ...
        if (!isWin) {
            ...
            this.pig_ske.playAnimation("ku", 0);
            AudioManager.playEffect("失败",false);
            this.scheduleOnce(() => {
                this.onGameFail();
                this.onlost();
            },1)
            ...
            // console.log("没有套到狼");
            this.onGameSucc();
            this.pig_ske.playAnimation("hh", 0);
            AudioManager.playEffect("成功",false);
                this.onwin();
    ...
    onwin() {
        this.iswin = true;
        this.gou.cleanup();
        const shou_ske = this.node.getChildByName("shou_ske");
        const shousk = shou_ske.getComponent(dragonBones.ArmatureDisplay);
        shou_ske.active = true;
        shousk.playAnimation("shou", 1);
        this.scheduleOnce(() => {
            this.pig_ske.playAnimation("chi2", 0);
        }, 1)
        this.scheduleOnce(() => {
            shou_ske.active = false;
        }, 2)
        cc.tween(this.gou)
    ...
    update() {
        if (this.m_isDrawFinish) {
            ...
            for (let i = 0; i < this.circleArr.length; i++) {
                ...
                this.circleArr[i].getComponent(CircleNode_CorrallingFlock308).onShorted(startPos, endPos);
            }
        ...
        while (worldPoints.length > 0) {
            ...
            while (nextPos != currPos) {
                ...
                if (sub.mag() >= this.m_circleDiameter) {
                    ...
                    circle.getComponent(CircleNode_CorrallingFlock308).setShepherd(this.shepherd);
                    // 设置距离关节
                    if (lastCircle) {
    ...
    public getCircleNode() {
        ...
        if (this.m_nodePool.size() > 0) {
            ...
            // const script = node.getComponent(RouteLogic_CorrallingFlock296);
            // script.ske = this.pig_ske;
            // if (script.ske) {
            //     return;
            // }
        }
