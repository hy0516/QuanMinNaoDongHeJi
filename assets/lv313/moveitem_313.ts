...
@ccclass
export default class moveitem_313 extends cc.Component {
    ...
    protected onLoad(): void {
        ...
        if(this.node.getChildByName("ske")){
    ...
    onTouchStart(even: cc.Event.EventTouch) {
        ...
        if(this.fske){
            this.sk.playAnimation("走路", 0);
        }
        this.wupinglan.getComponent(`moving_313`).moving = true;
    ...
    onTouchEnd(even: cc.Event.EventTouch) {
        ...
        if(this.fske){
            this.sk.playAnimation("待机", 0);
        }
        this.wupinglan.getComponent(`moving_313`).moving = false;
