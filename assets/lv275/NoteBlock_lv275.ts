import { NoteData, TIME_SCALE } from "./musicData_lv275";
 ...
 */
@ccclass
export default class NoteBlock_lv275 extends cc.Component {
    ...
    public init(noteData: NoteData, startX: number, targetX: number, generateTime: number) {
        ...
        // 设置颜色（根据noteData.color设置sprite颜色或使用不同的spriteFrame）
        this.setColor(noteData.color);
    ...
    /**
     * 设置颜色（加载对应的图片）
     * @param colorIndex 颜色索引 0-4 (绿、紫、黄、红、蓝)
     */
    private setColor(colorIndex: number) {
        ...
        // 颜色名称映射
        const colorNames = ["绿", "紫", "黄", "红", "蓝"];
        
        if (colorIndex < 0 || colorIndex >= colorNames.length) {
            console.warn(`无效的颜色索引: ${colorIndex}`);
            return;
        }
        
        // 路径格式：lv275/picture_lv275/notes/note_绿、note_紫、note_黄、note_红、note_蓝
        const colorName = colorNames[colorIndex];
        const notePath = `picture_lv275/notes/note_${colorName}`;
        ...
        AssetManager.load(
            ...
            (spriteFrame: cc.SpriteFrame) => {
                if (spriteFrame && sprite && sprite.node && sprite.node.isValid) {
                    ...
                    console.warn(`加载谱子图片失败: ${notePath}, colorIndex: ${colorIndex}`);
                }
            }
        );
    }
    ...
    public judge(checkPointX: number): "perfect" | "great" | "miss" | null {
        ...
        const perfectRange = nodeWidth / 4; 
        const greatRange = nodeWidth / 2; 
