...
//APPID 必填
const appId = "tt3f984925a09349b102"; //抖音小游戏小游戏ID
...
export default class DyHttpUpBehavior {
    ...
    */
    public queryEcpm() {
        ...
        const timeDate = cc.sys.localStorage.getItem("timeDate");
        if (timeDate) {
            //当缓存中的时间大于当前时间时
            if (this.convertToDate(todays) > this.convertToDate(timeDate)) {
                ...
                if (DyHttpUpBehavior.dy_click_id) {//当前用户为广告用户时
                    ...
                    let param = "open_id=" + DyHttpUpBehavior.dy_open_id + "&app_id=" + appId + "&click_id=queryEcpm";
                ...
                //更新缓存的年也日
                cc.sys.localStorage.setItem("timeDate", todays);
            } else {
