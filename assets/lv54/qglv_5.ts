...
@ccclass
export default class qglv_5 extends BaseGame {
    ...
    onLoad() {
        ...
        this.qiuStartPoi = cc.v2(-217.8, -8.8);
        this.Initlevel();
    ...
    Initlevel() {
        if (this.di && this.pic && this.rect) {
            ...
            // console.log(this.qiu.getComponent(cc.RigidBody).linearVelocity.x);
            this.curLevelId++;
        }
        if (this.qiu) {
            this.qiu.active = false;
            this.qiu = null;
        }
        this.qiu = this.qiuList[this.curLevelId - 1];
        this.qiu.active = true;
    ...
    onwin() {
        var fun = () => {
            ...
            this.gou.scale = 0;
            this.qiu.getComponent(cc.RigidBody).angularVelocity = 0;
            this.qiu.getComponent(cc.RigidBody).linearVelocity = cc.v2(0, 0);
            // this.qiu.getComponent(cc.RigidBody).enabled = false;
            // console.log(this.qiu.getComponent(cc.RigidBody).linearVelocity.x);
            this.IntBtn();
            this.meixi.getComponent(dragonBones.ArmatureDisplay).playAnimation("待机", -1);
    ...
    private splitTexture(polygons: cc.Vec2[][]) {
        ...
        this.offBtn();
        this.scheduleOnce(() => {
            this.meixi.getComponent(dragonBones.ArmatureDisplay).playAnimation("踢球", 1);
        }, 1.3)
        this.scheduleOnce(() => {
            this.meixi.getComponent(dragonBones.ArmatureDisplay).playAnimation("待机", -1);
        }, 2)
        this.scheduleOnce(() => {
            ...
            // this.qiu.getComponent(cc.RigidBody).enabled = true;
    ...
    ShowTime() {
        ...
        this.schedule(() => {
            if (time == 0) {
                ...
                if (this.isGameOver) {
                    AudioManager.playEffect(music.win);
                    this.meixi.getComponent(dragonBones.ArmatureDisplay).playAnimation("庆祝", -1);
                }
                time--
    ...
    onlost() {
        this.meixi.getComponent(dragonBones.ArmatureDisplay).playAnimation("失败", 2);
        this.scheduleOnce(() => {
            GameData.PauseGame = false
            this.node.parent.destroy();
